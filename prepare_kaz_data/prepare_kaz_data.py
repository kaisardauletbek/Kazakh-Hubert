...
num_processes = 64  # Adjust this based on your system
...
train_splits = ['crowdsourced', 'parliament', 'podcasts', 'radio', 'talkshow', 'tts', 'tv_news']
for split in train_splits:
    dataset = load_data_train("/raid/kaisar_dauletbek/datasets/ISSAI_KSC2/Train", split)
    print(f"Loaded {split} data")
    dataset = dataset.map(map_to_array)
    dataset = dataset.map(prepare_dataset, num_proc=num_processes, batch_size=16, batched=True)
